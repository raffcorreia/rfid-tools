...
The board has an unpopulated `J2` footprint near the left edge. Initial continuity testing confirms only the top square pad as `VCC` / `5V` and the bottom pad as `GND`.
